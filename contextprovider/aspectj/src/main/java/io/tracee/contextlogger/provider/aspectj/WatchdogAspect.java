...
import io.tracee.contextlogger.provider.aspectj.contextprovider.WatchdogDataWrapper;
...
import io.tracee.contextlogger.provider.aspectj.util.WatchdogUtils;
...
public class WatchdogAspect {
    ...
    @Pointcut("(execution(* *(..)) && @annotation(io.tracee.contextlogger.provider.aspectj.Watchdog))")
    void withinWatchdogAnnotatedMethods() {
    }

    @SuppressWarnings("unused")
    @Pointcut("within(@io.tracee.contextlogger.provider.aspectj.Watchdog *)")
