...
public class CommonDataProviderTest {
	...
	private void setSystemProperties(boolean stage, boolean system) {
		Mockito.doReturn(stage ? STAGE : null).when(commonDataContextProvider).getSystemProperty(CommonDataContextProvider.SYSTEM_PROPERTY_NAME_STAGE);
		Mockito.doReturn(system ? SYSTEM : null).when(commonDataContextProvider).getSystemProperty(CommonDataContextProvider.SYSTEM_PROPERTY_NAME_SYSTEM);
	}
