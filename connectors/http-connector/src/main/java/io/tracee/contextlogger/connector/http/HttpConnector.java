...
public class HttpConnector implements Connector {
    ...
    @Override
    public final void sendErrorReport(final ConnectorOutputProvider outputProvider) {

        final String targetUrl = this.url;
        String output = outputProvider.provideOutput();

        try {
            @SuppressWarnings("resource")
            final AsyncHttpClient asyncHttpClient = asyncHttpClientProvider.provideHttpClient(builder.build());
            asyncHttpClient.preparePost(targetUrl).setBody(output).setHeader("Content-type", "text/json;charset=utf-8").setBodyEncoding("UTF-8")
